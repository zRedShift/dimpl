...
fn dtls13_unknown_handshake_record(seq: u64) -> Vec<u8> {
    let mut handshake = Vec::new();
    handshake.push(0xff); // unknown handshake type
    handshake.extend_from_slice(&0u32.to_be_bytes()[1..]); // length
    handshake.extend_from_slice(&0u16.to_be_bytes()); // message_seq
    handshake.extend_from_slice(&0u32.to_be_bytes()[1..]); // fragment_offset
    handshake.extend_from_slice(&0u32.to_be_bytes()[1..]); // fragment_length
...
fn dtls13_bogus_then_client_hello_record(seq: u64, client_hello_packet: &[u8]) -> Vec<u8> {
...
fn dtls13_prehandshake_plaintext_non_client_hello_records_do_not_block_client_hello() {
    ...
    server
        .handle_packet(&dtls13_unknown_handshake_record(0))
        .expect("pre-handshake non-ClientHello handshake should not occupy receive queue");
...
#[test]
#[cfg(feature = "rcgen")]
fn dtls13_prehandshake_record_with_bogus_first_handshake_does_not_block_client_hello() {
    let _ = env_logger::try_init();
    ...
    server
        .handle_packet(&dtls13_bogus_then_client_hello_record(0, &f1[0]))
...
#[test]
#[cfg(feature = "rcgen")]
fn dtls13_prehandshake_mixed_future_epoch_record_does_not_block_client_hello() {
    let _ = env_logger::try_init();
    ...
    let mut poison = dtls13_ack_record(0);
    poison.extend_from_slice(&dtls13_future_epoch_ciphertext(0));
    server
...
#[test]
#[cfg(feature = "rcgen")]
fn dtls13_prehandshake_future_epoch_record_before_client_hello_is_filtered() {
    let _ = env_logger::try_init();
...
#[test]
#[cfg(feature = "rcgen")]
fn dtls13_prehandshake_too_many_filtered_records_still_fail() {
    let _ = env_logger::try_init();
    ...
    for seq in 1..=17 {
        packet.extend_from_slice(&dtls13_future_epoch_ciphertext(seq));
    }
